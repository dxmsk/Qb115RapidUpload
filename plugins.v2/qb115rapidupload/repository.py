...
class TaskRepository:
        ...
        with self._lock, self._connect() as connection:
            ...
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS tasks (
                    ...
                    organize_last_error TEXT,
                    organize_source_path TEXT,
                    organize_target_path TEXT,
    ...
    def source_missing_tasks(self, limit: int = 200) -> List[Dict[str, Any]]:
        """Return old source-missing tasks that may be recoverable after a path fix."""
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                """SELECT * FROM tasks WHERE status='ABANDONED_SOURCE_MISSING'
                   ORDER BY updated_at DESC LIMIT ?""",
                (max(1, int(limit)),),
            ).fetchall()
            return [dict(row) for row in rows]

    def recover_source_missing(self, task_id: int, paths: Dict[int, str]) -> bool:
        """Requeue a source-missing task when all files are found again."""
        if not paths:
            return False
        now = utcnow()
        with self._lock, self._connect() as connection:
            ...
            if not row or row["status"] != TaskStatus.ABANDONED_SOURCE_MISSING.value:
                return False
            for file_id, absolute_path in paths.items():
                connection.execute(
                    "UPDATE task_files SET absolute_path=?,last_error_code=NULL,last_error_message=NULL WHERE id=?",
                    (str(absolute_path), file_id),
                )
            updated = connection.execute(
                """UPDATE tasks SET status='WAITING',reason_code=NULL,cancel_requested=0,
                       next_retry_at=NULL,last_error_code=NULL,last_error_message=?,
                       updated_at=?,version=version+1 WHERE id=? AND status='ABANDONED_SOURCE_MISSING'""",
                ("路径解析修复后自动恢复秒传", now, task_id),
            ).rowcount
            if updated:
                self._event(
                    connection,
    ...
    def organize_candidates(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Return rapid-upload failures that have not entered organizing yet."""
        now = utcnow()
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                """SELECT * FROM tasks
                   WHERE status='RETRY_WAIT' AND cancel_requested=0
                     AND attempt_count > 0
                     AND (
                       organize_status='NONE'
                       OR (organize_status='FAILED' AND (
                           organize_next_retry_at IS NULL OR organize_next_retry_at <= ?
                       ))
                     )
    ...
    def mark_organize_queued(self, task_id: int, source_path: str) -> bool:
        now = utcnow()
        with self._lock, self._connect() as connection:
            updated = connection.execute(
                """UPDATE tasks SET organize_status='QUEUED',organize_queued_at=?,organize_source_path=?,
                       organize_next_retry_at=NULL,organize_last_error=NULL,updated_at=?,version=version+1
                   WHERE id=? AND status='RETRY_WAIT' AND cancel_requested=0
                     AND organize_status IN ('NONE','FAILED')""",
                (now, str(source_path or ""), now, task_id),
            ).rowcount
            if updated:
                self._event(connection, task_id, "ORGANIZE_QUEUED", detail={})
            return bool(updated)

    def mark_organize_submit_failed(
        self,
        task_id: int,
        message: str,
        retry_minutes: int = 5,
        ...
        now = now_dt.isoformat(timespec="seconds")
        retry_at = (
            now_dt + timedelta(minutes=max(1, int(retry_minutes)))
        ).isoformat(timespec="seconds")
        with self._lock, self._connect() as connection:
            updated = connection.execute(
                """UPDATE tasks SET organize_status='FAILED',organize_last_error=?,
                       organize_next_retry_at=?,updated_at=?,version=version+1
                   WHERE id=? AND organize_status='QUEUED'""",
                ((message or "整理入队失败")[:1000], retry_at, now, task_id),
            ).rowcount
            if updated:
                self._event(
                    connection,
                    task_id,
                    "ORGANIZE_SUBMIT_FAILED",
                    detail={"message": message, "retry_at": retry_at},
                )
            return bool(updated)

    def mark_organize_result(
        self,
        download_hash: str,
        success: bool,
        transfer_history_id: Optional[int] = None,
        ...
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                "SELECT id,organize_status FROM tasks WHERE download_hash=?",
                (normalized,),
            ).fetchall()
            changed = False
            for row in rows:
                updated = connection.execute(
                    """UPDATE tasks SET organize_status=?,organize_completed_at=?,
                           organize_next_retry_at=?,organize_last_error=?,transfer_history_id=?,
                           organized_result=?,organize_source_path=CASE WHEN ?<>'' THEN ? ELSE organize_source_path END,
                           organize_target_path=CASE WHEN ?<>'' THEN ? ELSE organize_target_path END,
                ...
                if updated:
                    ...
                    self._event(
                        connection,
                        row["id"],
                        "ORGANIZE_SUCCESS" if success else "ORGANIZE_FAILED",
                        detail={"message": message},
                    )
            return changed

    def mark_organize_cancelled_by_rapid(self, download_hash: str) -> bool:
        normalized = str(download_hash or "").strip().lower()
        if not normalized:
            return False
        now = utcnow()
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                """SELECT id FROM tasks WHERE download_hash=?
                   AND organize_status IN ('QUEUED','FAILED')""",
                (normalized,),
            ).fetchall()
            changed = False
            for row in rows:
                updated = connection.execute(
                    """UPDATE tasks SET organize_status='CANCELLED_RAPID_SUCCESS',
                           organize_completed_at=?,organize_next_retry_at=NULL,
                           organize_last_error=NULL,updated_at=?,version=version+1 WHERE id=?""",
                    (now, now, row["id"]),
                ).rowcount
                if updated:
                    changed = True
                    self._event(
                        connection,
                        row["id"],
                        "ORGANIZE_CANCELLED_RAPID_SUCCESS",
                    )
            return changed

    def organize_records(self, limit: int = 100) -> List[Dict[str, Any]]:
        with self._lock, self._connect() as connection:
            rows = connection.execute(
                """SELECT t.*,
                          COALESCE((SELECT SUM(expected_size) FROM task_files f WHERE f.task_id=t.id),0)
                            AS total_size
                   FROM tasks t WHERE organize_status<>'NONE'
                   ORDER BY COALESCE(organize_completed_at,organize_queued_at,updated_at) DESC
                   LIMIT ?""",
                (max(1, int(limit)),),
            ).fetchall()
            return [dict(row) for row in rows]
            ...
            rows = connection.execute(
                """SELECT t.*,
                          ...
                          (SELECT COALESCE(SUM(CASE
                              ...
                              WHEN f.expected_size >= 0 THEN f.expected_size ELSE 0 END),0)
                           FROM task_files f WHERE f.task_id=t.id) AS total_size,
                          (SELECT GROUP_CONCAT(DISTINCT f.remote_relative_dir)
                           FROM task_files f WHERE f.task_id=t.id AND f.remote_relative_dir<>'') AS remote_dirs
