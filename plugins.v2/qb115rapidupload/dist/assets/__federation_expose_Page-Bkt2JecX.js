...
function remotePath(item) {
  const cid = String(item.target_cid || '0');
  const storedPath = String(item.target_path || '').trim();
  const base = storedPath ? `115:${storedPath.startsWith('/') ? storedPath : `/${storedPath}`}` : cid === '0' ? '115:/' : `115:/目录ID/${cid}`;
  return item.remote_dirs ? `${base.replace(/\/$/, '')}/${item.remote_dirs}` : base
}
...
const Page = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-1dec6121"]]);
